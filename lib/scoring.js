// lib/scoring.js — Investment Scoring Engine v5
// Now uses per-character benchmarks instead of generic tier buckets.
//
...
const { getCharacterBenchmarks, getCharacterProfile } = require('./characters');

// Word-boundary name matching — prevents single-letter keys like 'N' from
// matching any card name containing that letter (e.g. "Sce-n-ario").
...
function getDemandScore(cardName) {
  ...
  for (const [p, t] of Object.entries(POKEMON_TIERS)) {
    if (matchesName(cardName, p)) { score = t; break; }
  }
  for (const [p, pen] of Object.entries(OVERPRINT_PENALTY)) {
    if (matchesName(cardName, p)) { score = Math.max(48, score - pen); break; }
  }
  return score;
}
...
function getPlayabilityContext(cardName) {
  for (const [card, status] of Object.entries(PLAYABILITY)) {
    if (matchesName(cardName, card)) return status;
  }
  return 'NONE';
}
...
// ─── PRICE DISLOCATION ────────────────────────────────────────────────────────
// Uses per-character benchmarks from lib/characters.js
// Each character has individual floor/fair/ceiling based on documented sales
// not generic tier buckets. Lt. Surge ≠ Cynthia even if both are trainer SIRs.
function getBenchmarks(rarity, demandScore, isTrainer, cardName) {
  // Use character-specific benchmarks if card name provided
  if (cardName) {
    const cb = getCharacterBenchmarks(cardName, rarity, isTrainer);
    return {
      floor: cb.floor,
      fair:  cb.fair,
      ceil:  cb.ceil,
      ...
      source: cb.source,
      profile: cb.profile,
      characterNotes: cb.characterNotes,
      collectorTier: cb.collectorTier,
    };
  }
  // Fallback: generic tier if no name (shouldn't happen in practice)
  const trainerBonus = isTrainer ? 1.35 : 1.0;
  const tier = demandScore >= 90 ? 't1' : demandScore >= 78 ? 't2' : demandScore >= 60 ? 't3' : 't4';
  const B = {
    'Special Illustration Rare': { t1:{floor:65,fair:140,ceil:320,peak:600}, t2:{floor:30,fair:78,ceil:175,peak:350}, t3:{floor:14,fair:38,ceil:90,peak:180}, t4:{floor:8,fair:20,ceil:50,peak:100} },
    'Hyper Rare':                { t1:{floor:38,fair:90,ceil:200,peak:400},  t2:{floor:20,fair:52,ceil:110,peak:220}, t3:{floor:9,fair:24,ceil:55,peak:110},  t4:{floor:5,fair:14,ceil:32,peak:65} },
    'Illustration Rare':         { t1:{floor:24,fair:58,ceil:130,peak:260},  t2:{floor:11,fair:30,ceil:68,peak:135},  t3:{floor:5,fair:16,ceil:38,peak:75},   t4:{floor:3,fair:9,ceil:22,peak:44} },
    'Ultra Rare':                { t1:{floor:14,fair:34,ceil:78,peak:150},   t2:{floor:7,fair:19,ceil:44,peak:88},    t3:{floor:3,fair:11,ceil:26,peak:52},   t4:{floor:2,fair:6,ceil:15,peak:30} },
  };
  const b = B[rarity]?.[tier] || { floor:6, fair:18, ceil:45, peak:90 };
  return { floor:Math.round(b.floor*trainerBonus), fair:Math.round(b.fair*trainerBonus), ceil:Math.round(b.ceil*trainerBonus), peak:Math.round(b.peak*trainerBonus), source:'generic' };
}
...
function getTrainerMultiplier(supertype, cardName, rarity) {
  ...
  if (rarity === 'Special Illustration Rare') {
    if (HIGH_TIER_TRAINERS.some(t => matchesName(cardName, t))) return 1.20;
    return 1.12;
  }
...
function scoreCard({ name, rarity, setId, releaseDate, printedTotal, price, supertype, ebayData }) {
  ...
  // Get character profile for frontend display
  const charProfile = getCharacterProfile(name);
  ...
  return {
    ...
    collectorTier: bench.collectorTier || charProfile?.collectorTier || 'unknown',
    characterNotes: bench.characterNotes || charProfile?.notes || null,
    reprintRiskScore: charProfile?.reprintRisk || null,
    breakdown: {
      supplyScore,
      demandScore,
      lifecycleScore,
