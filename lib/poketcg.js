...
function extractPrice(card) {
  ...
  if (p) {
    ...
    if (tcg > 0) return { price: tcg, source: 'tcgplayer' };
  }
  // Cardmarket fallback — trendPrice is 30-day moving avg; EUR ≈ USD within ~10%
  const cm = card.cardmarket?.prices;
  if (cm) {
    const est = +(cm.trendPrice || cm.averageSellPrice || cm.avg30 || 0).toFixed(2);
    if (est > 0) return { price: est, source: 'cardmarket' };
  }
  return { price: 0, source: 'none' };
}

function normalizeCard(card) {
  const _p = extractPrice(card);
  return {
    id:          card.id,
    name:        card.name,
    number:      card.number,
    ...
    marketPrice:  _p.price,
    priceSource:  _p.source,
...
// Fetch secret rares — supports page and sort rotation for shuffle feature
async function fetchSecretRares(rarities, page = 1, orderBy = '-set.releaseDate') {
  ...
  const safePage = Math.min(Math.max(1, parseInt(page) || 1), 4);
  const safeOrder = ['-set.releaseDate','name','-name','number','-number'].includes(orderBy)
    ? orderBy : '-set.releaseDate';
  const data = await apiFetch(
    `/cards?q=(${encodeURIComponent(q)})&orderBy=${safeOrder}&pageSize=250&page=${safePage}&select=${fields}`
  );
  const seen = new Set();
  return (data?.data || [])
